...
sites <- read.csv('Input/target_sites.csv')
...
# Average
average <- 60 # 0 means not doing average; for hourly data, "average" is set to 60
#--------------Customization--------------#
...
for (i in 1 : nrow(sites)) {
  ...
  for (j in 0 : as.numeric(end_date - start_date)) {
    ...
    # --- URL ---
    url.csv <- paste("https://thingspeak.com/channels/", channel, "/feed.csv?api_key=", key, "&offset=0&average=", average, "&round=3&start=", starttime, "&end=", endtime, sep = '')
